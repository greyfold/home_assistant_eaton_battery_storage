...
SENSOR_TYPES = {
    ...
    "status.energyFlow.acPvValue": {"name": "AC PV Value", "unit": UnitOfPower.WATT, "device_class": "power", "entity_category": None, "pv_related": True, "accuracy_warning": True},
    "status.energyFlow.batteryBackupLevel": {"name": "Battery Backup Level", "unit": PERCENTAGE, "device_class": None, "entity_category": EntityCategory.DIAGNOSTIC, "disabled_by_default": True},
    ...
    # WARNING: Inverter power measurements are typically 30% higher than actual values - accuracy is poor
    "status.energyFlow.criticalLoadValue": {"name": "Critical Load Value", "unit": UnitOfPower.WATT, "device_class": "power", "entity_category": None, "accuracy_warning": True},
    "status.energyFlow.dcPvRole": {"name": "DC PV Role", "unit": None, "device_class": None, "entity_category": None, "pv_related": True},
    # WARNING: Inverter power measurements are typically 30% higher than actual values - accuracy is poor
    "status.energyFlow.dcPvValue": {"name": "DC PV Value", "unit": UnitOfPower.WATT, "device_class": "power", "entity_category": None, "pv_related": True, "accuracy_warning": True},
    "status.energyFlow.gridRole": {"name": "Grid Role", "unit": None, "device_class": None, "entity_category": None},
    # WARNING: Inverter power measurements are typically 30% higher than actual values - accuracy is poor
    "status.energyFlow.gridValue": {"name": "Grid Power", "unit": UnitOfPower.WATT, "device_class": "power", "entity_category": None, "accuracy_warning": True},
    "status.energyFlow.nonCriticalLoadRole": {"name": "Non-Critical Load Role", "unit": None, "device_class": None, "entity_category": None},
    # WARNING: Inverter power measurements are typically 30% higher than actual values - accuracy is poor
    "status.energyFlow.nonCriticalLoadValue": {"name": "Non-Critical Load Value", "unit": UnitOfPower.WATT, "device_class": "power", "entity_category": None, "accuracy_warning": True},
    "status.energyFlow.operationMode": {"name": "Operation Mode", "unit": None, "device_class": None, "entity_category": None},
    # WARNING: Inverter power measurements are typically 30% higher than actual values - accuracy is poor
    "status.energyFlow.selfConsumption": {"name": "Self Consumption", "unit": UnitOfPower.WATT, "device_class": "power", "entity_category": None, "accuracy_warning": True},
    ...
    "status.last30daysEnergyFlow.gridConsumption": {"name": "30 Days Grid Consumption", "unit": UnitOfPower.WATT, "device_class": "power", "entity_category": None, "disabled_by_default": True, "accuracy_warning": True},
    "status.last30daysEnergyFlow.photovoltaicProduction": {"name": "30 Days PV Production", "unit": UnitOfPower.WATT, "device_class": "power", "entity_category": None, "pv_related": True, "disabled_by_default": True, "accuracy_warning": True},
    "status.last30daysEnergyFlow.selfConsumption": {"name": "30 Days Self Consumption", "unit": PERCENTAGE, "device_class": None, "entity_category": None, "disabled_by_default": True, "accuracy_warning": True},
    "status.last30daysEnergyFlow.selfSufficiency": {"name": "30 Days Self Sufficiency", "unit": PERCENTAGE, "device_class": None, "entity_category": None, "disabled_by_default": True, "accuracy_warning": True},
    # WARNING: Today's metrics also affected by inverter accuracy issues
    "status.today.gridConsumption": {"name": "Today's Grid Consumption", "unit": UnitOfPower.WATT, "device_class": "power", "entity_category": None, "disabled_by_default": True, "accuracy_warning": True},
    "status.today.photovoltaicProduction": {"name": "Today's PV Production", "unit": UnitOfPower.WATT, "device_class": "power", "entity_category": None, "pv_related": True, "accuracy_warning": True},
    "status.today.selfConsumption": {"name": "Today's Self Consumption", "unit": PERCENTAGE, "device_class": None, "entity_category": None, "disabled_by_default": True, "accuracy_warning": True},
    "status.today.selfSufficiency": {"name": "Today's Self Sufficiency", "unit": PERCENTAGE, "device_class": None, "entity_category": None, "disabled_by_default": True, "accuracy_warning": True},
    # device endpoint
    ...
    "technical_status.dcCurrentInjectionR": {"name": "DC Current Injection R", "unit": "A", "device_class": "current", "entity_category": EntityCategory.DIAGNOSTIC, "pv_related": True},
    "technical_status.dcCurrentInjectionS": {"name": "DC Current Injection S", "unit": "A", "device_class": "current", "entity_category": EntityCategory.DIAGNOSTIC, "pv_related": True},
    "technical_status.dcCurrentInjectionT": {"name": "DC Current Injection T", "unit": "A", "device_class": "current", "entity_category": EntityCategory.DIAGNOSTIC, "pv_related": True},
...
class EatonXStorageSensor(CoordinatorEntity, SensorEntity):
    ...
    def state(self):
        try:
            ...
            if self._accuracy_warning:
                _LOGGER.debug(f"Sensor {self._name} ({self._key}) - {POWER_ACCURACY_WARNING}")
            ...
            if self._key.startswith("technical_status.") and value is not None:
                _LOGGER.debug(f"Technical Status Sensor '{self._key}' - Raw value: '{value}' (type: {type(value).__name__})")
            ...
            if self._key == "status.currentMode.parameters.action" and isinstance(value, str):
            ...
            if self._key == "status.energyFlow.operationMode" and isinstance(value, str):
            ...
            if self._key == "technical_status.operationMode" and isinstance(value, str):
                ...
                _LOGGER.debug(f"Technical Operation Mode - Raw value: '{value}', Mapped value: '{OPERATION_MODE_MAP.get(value, value)}'")
                return OPERATION_MODE_MAP.get(value, value)
            ...
            if self._key == "status.energyFlow.batteryStatus" and isinstance(value, str):
                return BMS_STATE_MAP.get(value, value)
    ...
    def suggested_display_precision(self):
        ...
        if self._device_class == "temperature":
            return 1
        # Voltage sensors: 1 decimal place  
        elif self._device_class == "voltage":
            return 1
        # Current sensors: 2 decimal places
        elif self._device_class == "current":
            return 2
        # Frequency sensors: 2 decimal places
        elif self._device_class == "frequency":
            return 2
        # Power sensors: 0 decimal places (whole watts)
        elif self._device_class == "power":
            return 0
        # Energy sensors: 1 decimal place
        elif self._device_class == "energy" or self._device_class == "energy_storage":
            return 1
        # Apparent power (VA): 0 decimal places
        elif self._device_class == "apparent_power":
            return 0
        # Percentage sensors: 0 decimal places
        elif self._unit == PERCENTAGE:
            return 0
        # RAM usage (MB): 0 decimal places
        elif "ramUsage" in self._key:
            return 0
        # CPU usage: 1 decimal place
        elif "cpuUsage" in self._key:
            return 1
        # Cell voltage sensors (mV): 0 decimal places (already in millivolts)
        elif "CellVoltage" in self._key or "VoltageDelta" in self._key:
            return 0
